  ...
  preload() {
    ...
    this.load.once("complete", () => {
      ...
      const _startGame = () => {
        ...
        const bigFontData = _scene.cache.text.get("bigFontFnt");
        if (bigFontData) loadFont(_scene, "bigFont", bigFontData);
        const gfd = _scene.cache.text.get("goldFontFnt");
        if (gfd && !_scene.cache.bitmapFont.has("goldFont")) loadFont(_scene, "goldFont", gfd);
        localStorage.setItem('webdash_assets_loaded', 'true');
        localStorage.setItem('webdash_last_load_time', Date.now().toString());
        _scene.scene.start("GameScene");
      };

      const _loadTimeoutId = setTimeout(() => {
        if (_scene.scene.isActive('BootScene')) _startGame();
      }, 20000);

      this.load.once("complete", () => {
        this.time.delayedCall(200, _startGame);
      });
